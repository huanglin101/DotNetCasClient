    ...
    class Cas20ServiceTicketValidator : AbstractCasProtocolTicketValidator
    {
        ...
        protected override ICasPrincipal ParseResponseFromServer(string response, string ticket)
        {
            ...
            if (serviceResponse.IsAuthenticationSuccess)
            {
                ...
                Attributes model = authSuccessResponse.attributes as Attributes;
                IDictionary<string, IList<string>> attributes =new  Dictionary<string, IList<string>>() {
                    { "userId",new List<string>() { model.userId} } ,
                    { "name",new List<string>() { model.name} } ,                  
                };
                if (authSuccessResponse.Proxies != null && authSuccessResponse.Proxies.Length > 0)
                {
                    return new CasPrincipal(new Assertion(authSuccessResponse.User, attributes), proxyGrantingTicketIou, authSuccessResponse.Proxies);
                } 
                else
                {
